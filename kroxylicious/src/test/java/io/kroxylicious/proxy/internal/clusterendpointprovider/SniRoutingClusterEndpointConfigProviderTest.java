...
class SniRoutingClusterEndpointConfigProviderTest {
    ...
    void findsBootstrapMatch(@ConvertWith(HostPortConverter.class) HostPort data) {
        var provider = new SniRoutingClusterEndpointConfigProvider(new SniRoutingClusterEndpointProviderConfig(parse("boot:1234"), "unused$(nodeId)"));
    ...
    @Test
    void noSni() {
        var provider = new SniRoutingClusterEndpointConfigProvider(new SniRoutingClusterEndpointProviderConfig(parse("good:1235"), "broker$(nodeId)-good"));
