...
/**
 * A stateless filter for LeaderChangeMessages.
 * The same instance may be invoked on multiple channels.
 */
public interface LeaderChangeMessageFilter {

    /**
     * Handle the given {@code data},
     * returning the {@code LeaderChangeMessageData} instance to be passed to the next filter.
     ...
     * @return the {@code LeaderChangeMessageData} instance to be passed to the next filter.
     * If null is returned then the given {code data} will be used.
     */
    public LeaderChangeMessageData onLeaderChangeMessage(LeaderChangeMessageData data, FilterContext context);
}