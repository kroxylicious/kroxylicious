...
public class KeyVaultClient implements AutoCloseable {
    ...
    public KeyVaultClient(BearerTokenService service, AzureKeyVaultConfig config) {
        ...
        this.client = builder.version(HttpClient.Version.HTTP_1_1).followRedirects(HttpClient.Redirect.NEVER).connectTimeout(Duration.of(10, ChronoUnit.SECONDS)).build();
    }
    ...
    private HttpRequest getKeyRequest(String keyName, BearerToken bearerToken) {
        String getKey = config.keyVaultBaseUrl() + "/keys/" + keyName + "?api-version=7.4";
    ...
    private HttpRequest wrapOrUnwrapKeyRequest(WrappingKey wrappingKey, byte[] bytes, BearerToken bearerToken, String operation) {
        String wrapKey = config.keyVaultBaseUrl() + "/keys/" + wrappingKey.keyName() + "/" + wrappingKey.keyVersion() + "/" + operation + "?api-version=7.4";
