...
public record GetKeyResponse(@JsonProperty(required = true) JsonWebKey key, @JsonProperty(required = true) KeyAttributes attributes) {
    ...
    /**
     * @return if this key type is supported, return it's key type, else empty
     */
    public Optional<SupportedKeyType> supportedKeyType() {
        return SupportedKeyType.fromKeyType(key().keyType());
    }
}
