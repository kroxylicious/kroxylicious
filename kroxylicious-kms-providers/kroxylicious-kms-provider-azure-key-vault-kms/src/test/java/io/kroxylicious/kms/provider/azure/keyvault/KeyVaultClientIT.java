...
class KeyVaultClientIT {
    ...
    LowkeyVaultContainer startVault() {
        // bit annoying to have to sync the image name
    ...
    void getKey() {
        try (LowkeyVaultContainer lowkeyVaultContainer = startVault()) {
            ...
            KeyVaultClient keyVaultClient = new KeyVaultClient(bearerTokenService,
                    new AzureKeyVaultConfig(unused, lowkeyVaultContainer.getVaultBaseUrl(VAULT_NAME), INSECURE_TLS));
            ...
            assertThat(keyVaultClient.getKey(KEY_NAME).toCompletableFuture()).succeedsWithin(Duration.ofSeconds(10)).satisfies(getKeyResponse -> {
                ...
                assertThat(getKeyResponse.supportedKeyType()).contains(SupportedKeyType.RSA);
