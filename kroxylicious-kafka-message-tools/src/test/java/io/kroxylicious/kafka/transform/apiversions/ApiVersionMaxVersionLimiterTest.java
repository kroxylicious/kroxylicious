...
class ApiVersionMaxVersionLimiterTest extends ApiVersionsInterceptorTester {
    ...
    void testApiVersionMaxVersionLimiter(short maxVersionLimit, short inputMinVersion, short inputMaxVersion, short expectedMinVersion, short expectedMaxVersion) {
        ...
        ApiVersionsResponseData apiVersionsResponseData = limiter.onApiVersionsResponse(input);
    ...
    void throwsIfMinimumVersionIsAboveTheLimit() {
        ...
        assertThatThrownBy(() -> {
            limiter.onApiVersionsResponse(input);
        }).isInstanceOf(ApiVersionsInterceptionException.class)
                .hasMessage("upstream advertised min version 6 for PRODUCE is above the limit of 5imposed by this interceptor");
    }

    @Test
    void preventTopicIdBearingProduceRequests() {
        ApiVersionsResponseTransformer limiter = ApiVersionsResponseTransformers.preventTopicIdBearingProduceRequests();
        ApiVersionsResponseData input = apiVersionsResponseData(apiVersion(ApiKeys.PRODUCE, (short) 0, (short) 13));
        ApiVersionsResponseData apiVersionsResponseData = limiter.onApiVersionsResponse(input);
        assertVersionsForApiKey(apiVersionsResponseData, ApiKeys.PRODUCE, (short) 0, (short) 12);
    }
    ...
    void andChains() {
        ...
        ApiVersionsResponseTransformer interceptor = limiter.and(limiter2);
