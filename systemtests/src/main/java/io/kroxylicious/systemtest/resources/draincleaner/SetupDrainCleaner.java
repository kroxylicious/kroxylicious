...
import io.kroxylicious.systemtest.security.CertAndKeyFiles;
import io.kroxylicious.systemtest.security.SystemTestCertAndKey;
import io.kroxylicious.systemtest.security.SystemTestCertManager;
                ...
                .generateRootCaCertAndKey("C=CZ, L=Prague, O=Strimzi Drain Cleaner, CN=StrimziDrainCleanerCA",
                        ...
                        new ASN1Encodable[]{
                                ...
                                new GeneralName(GeneralName.dNSName,
                                        Constants.DRAIN_CLEANER_DEPLOYMENT_NAME + "." + Constants.DRAIN_CLEANER_DEPLOYMENT_NAME + ".svc.cluster.local")
                        });
            ...
            if (!file.getName().contains("README") && !file.getName().contains("Namespace") && !file.getName().contains("Deployment")) {
                ...
                switch (resourceType) {
                    ...
                    case Constants.VALIDATION_WEBHOOK_CONFIG:
                        ...
                        webhookConfiguration.getWebhooks().stream().findFirst().get().getClientConfig()
                                .setCaBundle(customDrainCleanerSecretBuilder.getData().get("tls.crt"));
