 ...
 */
package io.kroxylicious.systemtest.resources.operator.specific;
...
import io.kroxylicious.systemtest.enums.OlmInstallationStrategy;
...
import static io.kroxylicious.systemtest.resources.ResourceManager.CR_CREATION_TIMEOUT;
...
public class OlmResource implements SpecificResourceType {
    ...
    private Map<String, JsonObject> parseExamplesFromCsv() {
        JsonArray examples = new JsonArray(kubeClient().getCsvWithPrefix(olmConfiguration.getNamespaceName(), olmConfiguration.getOlmAppBundlePrefix()).getMetadata()
                .getAnnotations().get("alm-examples"));
    ...
    public void deleteCSV() {
        ...
        if (olmConfiguration.getOperatorVersion() == null) {
            kubeClient().deleteCsv(olmConfiguration.getNamespaceName(),
                    kubeClient().getCsvWithPrefix(olmConfiguration.getNamespaceName(), olmConfiguration.getOlmAppBundlePrefix()).getMetadata().getName());
        }
        else {
