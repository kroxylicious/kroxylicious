...
import io.kroxylicious.systemtest.listeners.ExecutionListener;
...
public class TestSuiteNamespaceManager {
     ...
     * contains {@link io.kroxylicious.systemtest.annotations.IsolatedTest} or {@link io.kroxylicious.systemtest.annotations.ParallelTest}
     * test cases we will create such namespace, otherwise not). When we have test class, which only contains
     *  {@link io.kroxylicious.systemtest.annotations.ParallelNamespaceTest}, which creates its own namespace it does not
        ...
        if (StUtils.isParallelNamespaceTest(extensionContext)) {
            ...
            if (!Environment.isNamespaceRbacScope()) {
                ...
                KubeClusterResource.getInstance()
                        .createNamespace(CollectorElement.createCollectorElement(extensionContext.getRequiredTestClass().getName(), testCaseName), namespaceTestCase);
    ...
    public void deleteParallelNamespace(ExtensionContext extensionContext) {
        ...
        if (StUtils.isParallelNamespaceTest(extensionContext)) {
            ...
            if (!Environment.isNamespaceRbacScope()) {
                ...
                KubeClusterResource.getInstance()
                        .deleteNamespace(CollectorElement.createCollectorElement(extensionContext.getRequiredTestClass().getName(), testCaseName), namespaceToDelete);
            }
        }
    }
}
