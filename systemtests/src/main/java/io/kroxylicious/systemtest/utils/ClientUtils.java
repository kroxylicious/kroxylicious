...
import io.kroxylicious.systemtest.utils.kubeUtils.controllers.JobUtils;
import io.kroxylicious.systemtest.utils.kubeUtils.objects.PodUtils;

import static io.kroxylicious.systemtest.resources.ResourceManager.kubeClient;

/**
 * ClientUtils class, which provides static methods for the all type clients
 * @see io.kroxylicious.systemtest.kafkaclients.externalClients.ExternalKafkaClient
 */
public class ClientUtils {
    ...
    public static void waitForClientsSuccess(TestStorage testStorage, boolean deleteAfterSuccess) {
        waitForClientsSuccess(testStorage.getProducerName(), testStorage.getConsumerName(), testStorage.getNamespaceName(), testStorage.getMessageCount(),
                deleteAfterSuccess);
    }
    ...
    public static void waitForClientTimeout(String jobName, String namespace, int messageCount, boolean deleteAfterSuccess) {
        ...
        try {
            TestUtils.waitFor("client Job: " + namespace + "/" + jobName + "to reach the the timeout limit", Constants.GLOBAL_POLL_INTERVAL,
                    timeoutForClientFinishJob(messageCount),
    ...
    public static void waitForClientsTimeout(String producerName, String consumerName, String namespace, int messageCount, boolean deleteAfterSuccess) {
        ...
        try {
            TestUtils.waitFor("client Jobs: " + producerName + " and " + consumerName + " in Namespace: " + namespace + " to reach the timeout limit",
                    Constants.GLOBAL_POLL_INTERVAL, timeoutForClientFinishJob(messageCount),
