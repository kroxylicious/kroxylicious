 ...
 */
package io.kroxylicious.systemtest.utils.kubeUtils.controllers;
...
public class StrimziPodSetUtils {
    ...
    public static void waitForStrimziPodSetLabelsDeletion(String namespaceName, String resourceName, Collection<String> labelKeys) {
        for (final String labelKey : labelKeys) {
            ...
            TestUtils.waitFor("StrimziPodSet: " + namespaceName + "/" + resourceName + " to change label: " + labelKey + " -> " + null,
                    Constants.POLL_INTERVAL_FOR_RESOURCE_READINESS,
                    DELETION_TIMEOUT, () -> StrimziPodSetResource.strimziPodSetClient().inNamespace(namespaceName).withName(resourceName).get().getMetadata().getLabels()
                            .get(labelKey) == null);
    ...
    public static void waitForStrimziPodSetLabelsChange(String namespaceName, String resourceName, Map<String, String> labels) {
        for (Map.Entry<String, String> entry : labels.entrySet()) {
            ...
            if (!(isStrimziTag || isK8sTag)) {
                ...
                TestUtils.waitFor("StrimziPodSet: " + namespaceName + "/" + resourceName + " to change label: " + entry.getKey() + " -> " + entry.getValue(),
                        Constants.POLL_INTERVAL_FOR_RESOURCE_READINESS,
                        Constants.GLOBAL_TIMEOUT, () -> StrimziPodSetResource.strimziPodSetClient().inNamespace(namespaceName).withName(resourceName).get().getMetadata()
    ...
    public static void waitForStrimziPodSetRecovery(String namespaceName, String resourceName, String resourceUID) {
        ...
        TestUtils.waitFor("readiness of StrimziPodSet: " + namespaceName + "/" + resourceName, Constants.POLL_INTERVAL_FOR_RESOURCE_READINESS,
