 ...
 */
package io.kroxylicious.systemtest.utils.kafkaUtils;
...
import io.kroxylicious.systemtest.cli.KafkaCmdClient;
...
import io.kroxylicious.systemtest.resources.crd.StrimziPodSetResource;
...
import io.kroxylicious.systemtest.utils.kubeUtils.objects.PodUtils;

import static io.kroxylicious.systemtest.enums.CustomResourceStatus.NotReady;
...
public class KafkaUtils {
     ...
     */
    public synchronized static boolean verifyPodDynamicConfiguration(final String namespaceName, String scraperPodName, String bootstrapServer, String kafkaPodNamePrefix,
                                                                     String brokerConfigName, Object value) {
        ...
        TestUtils.waitFor("deletion of Kafka: " + namespaceName + "/" + kafkaClusterName, Constants.POLL_INTERVAL_FOR_RESOURCE_READINESS, DELETION_TIMEOUT,
                () -> {
                    if (KafkaResource.kafkaClient().inNamespace(namespaceName).withName(kafkaClusterName).get() == null &&
                            StrimziPodSetResource.strimziPodSetClient().inNamespace(namespaceName).withName(KafkaResources.kafkaStatefulSetName(kafkaClusterName))
                                    .get() == null
                            &&
                            StrimziPodSetResource.strimziPodSetClient().inNamespace(namespaceName).withName(KafkaResources.zookeeperStatefulSetName(kafkaClusterName))
                                    .get() == null
                            &&
    ...
    public static String getKafkaTlsListenerCaCertName(String namespace, String clusterName, String listenerName) {
        ...
        return tlsListener.getConfiguration() == null ? KafkaResources.clusterCaCertificateSecretName(clusterName)
                : tlsListener.getConfiguration().getBrokerCertChainAndKey().getSecretName();
    }
    ...
    public static String getKafkaExternalListenerCaCertName(String namespace, String clusterName, String listenerName) {
        ...
        GenericKafkaListener external = listenerName == null || listenerName.isEmpty()
                ? listeners.stream().filter(listener -> Constants.EXTERNAL_LISTENER_DEFAULT_NAME.equals(listener.getName())).findFirst()
