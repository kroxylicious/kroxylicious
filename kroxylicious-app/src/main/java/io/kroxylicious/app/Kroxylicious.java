...
public class Kroxylicious implements Callable<Integer> {
    ...
    private final BiFunction<FilterFactoryManager, Configuration, KafkaProxy> proxyBuilder;

    Kroxylicious() {
        this(KafkaProxy::new);
    }

    Kroxylicious(BiFunction<FilterFactoryManager, Configuration, KafkaProxy> proxyBuilder) {
    ...
    public Integer call() throws Exception {
        ...
        try (InputStream stream = Files.newInputStream(configFile.toPath())) {
            Configuration config = new ConfigParser(ffm).parseConfiguration(stream);
            try (KafkaProxy kafkaProxy = proxyBuilder.apply(ffm, config)) {
                kafkaProxy.startup();
