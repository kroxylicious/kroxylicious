...
Instead, we apply the enrichment pattern at the runtime level via two internal filters installed early in the pipeline. Everything after them — user filters, routers, other internal filters — always sees topic names populated, regardless of API version.
...
Added `apiVersion` parameter to `RequestDecomposer.decompose()` and `recompose()` so decomposers can handle version-dependent wire formats. Also added a `Function<Uuid, String> topicNameResolver` parameter to `decompose()` so decomposers can resolve topicIds when the wire object has no name field. Decomposers with topicId fields (Produce, Fetch, DeleteTopics, OffsetCommit, OffsetFetch) fall back to the resolver when the name is null/empty. A default 3-arg overload provides backwards compatibility.

### Synchronous topicName lookup on RouterContext

The enrichment pattern works for APIs that have both `topicId` and `topicName` fields, but newer Kafka APIs (SHARE_FETCH, SHARE_ACKNOWLEDGE, Share Group state APIs) have **only** a `topicId` — no name property exists on the wire object. `RouterContext.topicName(Uuid)` provides a synchronous lookup backed by the same per-connection cache that `TopicIdRequestEnrichmentFilter` populates. The `HashMap` is created in `KafkaProxyFrontendHandler` and shared between the filter (writes) and `RouterContextImpl` (reads). By the time `Router.onRequest()` runs, the cache is guaranteed warm. `TopicPartitionRouter` passes `context::topicName` to all decomposer call sites.
...
- [ ] 148 `KafkaProxyFrontendHandlerTest` tests pass (including METADATA v13 error path)
- [ ] 3 `RouterContextImplTest` tests pass (including `topicName` cache lookup)
