...
import io.kroxylicious.testing.filter.record.VendoredRecordTestUtils;
 ...
 */
public class VendoredRequestFactory {
    ...
    private static final byte CLIENT_METRICS_RESOURCE_TYPE = (byte) 16;

    static {
        messagePopulators.put(ApiKeys.PRODUCE, VendoredRequestFactory::populateProduceRequest);
        messagePopulators.put(ApiKeys.LIST_OFFSETS, VendoredRequestFactory::populateListOffsetsRequest);
        messagePopulators.put(ApiKeys.OFFSET_FETCH, VendoredRequestFactory::populateOffsetFetchRequest);
        messagePopulators.put(ApiKeys.METADATA, VendoredRequestFactory::populateMetadataRequest);
        messagePopulators.put(ApiKeys.LEAVE_GROUP, VendoredRequestFactory::populateLeaveGroupRequest);
        messagePopulators.put(ApiKeys.DESCRIBE_GROUPS, VendoredRequestFactory::populateDescribeGroupsRequest);
        messagePopulators.put(ApiKeys.CONSUMER_GROUP_DESCRIBE, VendoredRequestFactory::populateConsumeGroupDescribeRequest);
        messagePopulators.put(ApiKeys.DELETE_GROUPS, VendoredRequestFactory::populateDeleteGroupRequest);
        messagePopulators.put(ApiKeys.OFFSET_COMMIT, VendoredRequestFactory::populateOffsetCommitRequest);
        messagePopulators.put(ApiKeys.CREATE_TOPICS, VendoredRequestFactory::populateCreateTopicsRequest);
        messagePopulators.put(ApiKeys.DELETE_TOPICS, VendoredRequestFactory::populateDeleteTopicsRequest);
        messagePopulators.put(ApiKeys.DELETE_RECORDS, VendoredRequestFactory::populateDeleteRecordsRequest);
        messagePopulators.put(ApiKeys.INIT_PRODUCER_ID, VendoredRequestFactory::populateInitProducerIdRequest);
        messagePopulators.put(ApiKeys.CREATE_ACLS, VendoredRequestFactory::populateCreateAclsRequest);
        messagePopulators.put(ApiKeys.DESCRIBE_ACLS, VendoredRequestFactory::populateDescribeAclsRequest);
        messagePopulators.put(ApiKeys.DELETE_ACLS, VendoredRequestFactory::populateDeleteAclsRequest);
        messagePopulators.put(ApiKeys.OFFSET_FOR_LEADER_EPOCH, VendoredRequestFactory::populateOffsetForLeaderEpochRequest);
        messagePopulators.put(ApiKeys.SHARE_GROUP_DESCRIBE, VendoredRequestFactory::populateShareGroupDescribeRequest);
        messagePopulators.put(ApiKeys.INITIALIZE_SHARE_GROUP_STATE, VendoredRequestFactory::populateInitializeShareGroupStateRequest);
        messagePopulators.put(ApiKeys.READ_SHARE_GROUP_STATE, VendoredRequestFactory::populateReadShareGroupStateRequest);
        messagePopulators.put(ApiKeys.WRITE_SHARE_GROUP_STATE, VendoredRequestFactory::populateWriteShareGroupStateRequest);
        messagePopulators.put(ApiKeys.DELETE_SHARE_GROUP_STATE, VendoredRequestFactory::populateDeleteShareGroupStateRequest);
        messagePopulators.put(ApiKeys.READ_SHARE_GROUP_STATE_SUMMARY, VendoredRequestFactory::populateReadShareGroupStateSummaryRequest);
        messagePopulators.put(ApiKeys.STREAMS_GROUP_DESCRIBE, VendoredRequestFactory::populateStreamsGroupDescribeRequest);
        messagePopulators.put(ApiKeys.DESCRIBE_SHARE_GROUP_OFFSETS, VendoredRequestFactory::populateDescribeShareGroupOffsetsRequest);
        messagePopulators.put(ApiKeys.LIST_CONFIG_RESOURCES, VendoredRequestFactory::populateListConfigResourcesRequest);
    }

    private VendoredRequestFactory() {
    }
    ...
    public static Set<ApiKeys> supportedApiKeys() {
        var excluded = VendoredRequestFactory.SPECIAL_CASES;
    ...
    private static void populateProduceRequest(ApiMessage apiMessage, short apiVersion) {
        ...
        produceData.setRecords(VendoredRecordTestUtils.memoryRecords(List.of(VendoredRecordTestUtils.record(MobyNamesGenerator.getRandomName()))));
    ...
    private static void populateCreateAclsRequest(ApiMessage apiMessage, short apiVersion) {
        ...
        aclCreation.setResourcePatternType((byte) 3); // Literal
        aclCreation.setOperation((byte) 1); // ANY
        aclCreation.setPermissionType((byte) 1); // ANY
        aclCreation.setResourceType((byte) 1); // ANY
    ...
    private static void populateDescribeAclsRequest(ApiMessage apiMessage, short apiVersion) {
        ...
        describeAclsRequestData.setPatternTypeFilter((byte) 1); // ANY
        describeAclsRequestData.setOperation((byte) 1); // ANY
        describeAclsRequestData.setPermissionType((byte) 1); // ANY
        describeAclsRequestData.setResourceNameFilter(MobyNamesGenerator.getRandomName());
        describeAclsRequestData.setResourceTypeFilter((byte) 1); // ANY
    }
    ...
    private static void populateListConfigResourcesRequest(ApiMessage apiMessage, short apiVersion) {
        ...
        if (apiVersion == 0) {
            ...
            listConfigResourcesRequestData.setResourceTypes(List.of(CLIENT_METRICS_RESOURCE_TYPE));
        }
    }
