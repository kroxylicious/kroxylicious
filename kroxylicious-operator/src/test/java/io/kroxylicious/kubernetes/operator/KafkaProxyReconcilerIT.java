...
class KafkaProxyReconcilerIT {
    ...
    void clusterIpIngressUsesDeclaredNodeIdsOfService() {
        ...
        AWAIT.alias("service configured with a port per node id from the KafkaService, plus a bootstrap port").untilAsserted(() -> {
            ...
            assertThat(service.getSpec().getPorts()).containsExactly(createServicePort(CLUSTER_BAR + "-" + 9292, 9292), createServicePort(CLUSTER_BAR + "-" + 9293, 9293),
                    createServicePort(CLUSTER_BAR + "-" + 9294, 9294), createServicePort(CLUSTER_BAR + "-" + 9295, 9295));
        });
        ...
        AWAIT.alias("deployment pod template configured to expose ports").untilAsserted(() -> {
            ...
            assertThat(containers).hasSize(1).singleElement().satisfies(container -> {
                ...
                assertThat(container.getPorts())
                        .contains(createContainerPort(9292, 9292 + "-bootstrap"),
                                createContainerPort(9293, 9293 + "-node"),
                                createContainerPort(9294, 9294 + "-node"),
                                createContainerPort(9295, 9295 + "-node"));
            });
        });
