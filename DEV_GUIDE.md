...
the `component-build` property is absent, so a plain `mvn verify` builds everything. IDEs pick this up
without any extra configuration. For isolated builds, pass `-Dcomponent-build` and name the group(s) you want:

```shell
mvn -Dcomponent-build -Pproxy-core verify              # proxy modules only
mvn -Dcomponent-build -Pkubernetes-management verify   # kubernetes modules only
```

| profile                  | modules included                                                                                        |
|--------------------------|---------------------------------------------------------------------------------------------------------|
| `build-the-world`        | All groups — activated when `component-build` property is absent. Pass `-Dcomponent-build` to exclude it.          |
