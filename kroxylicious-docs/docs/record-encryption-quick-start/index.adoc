...
[source,terminal,subs="attributes"]
----
$ kubectl apply -f {OperatorInstallManifestLink}
...
[source,terminal,subs="attributes"]
----
$ curl --fail --location --silent {OperatorExamplesZipLink} --output kroxylicious-operator-examples.zip
$ unzip -q kroxylicious-operator-examples.zip
----
...
[source,terminal]
----
$ kubectl apply -f record-encryption/
----
...
----
$ kubectl patch -n my-proxy kafkaprotocolfilters.kroxylicious.io encryption --patch '{"spec":{"configTemplate":{"kms":"AwsKmsService","kmsConfig":{"credentials":{"longTerm":{"accessKeyId":{"password":"unused"},"secretAccessKey": {"password":"unused"}}},"endpointUrl":"http://localstack.kms.svc.cluster.local:4566/", "region" : "us-east-1"}, "selector" : "TemplateKekSelector", "selectorConfig" : {"template": "KEK-$(topicName)"}}}}' --type merge
----
====
...
[source,terminal,subs="attributes"]
----
$ kubectl delete -f record-encryption/
...
[source,terminal,subs="attributes"]
----
$ kubectl delete -f {OperatorInstallManifestLink}
