...
The built-in credential file store is named `ScramCredentialFileService`.
...
=== Credential file store configuration

The `ScramCredentialFileService` credential store loads SCRAM credentials from a proxy SCRAM credential file.
...
* `filePassword` (required) specifies the password used to access the SCRAM credential file, using a `PasswordProvider`:
