...
* `wireFormatVersion` controls whether the Apicurio client operates in `V3` (default) or `V2` (deprecated).
   In `V3` mode, the `apicurioId` property identifies schema by content ID. In this mode, the filter expects the Kafka producer to identify schemas by content ID.
   In `V2` mode, the `apicurioId` property identifies schema by global ID. In this mode, the filter expects the Kafka producer to identify schemas by global ID.
