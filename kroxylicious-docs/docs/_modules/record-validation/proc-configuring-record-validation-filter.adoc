...
* `wireFormatVersion` controls whether the Apicurio client operates in `V3` (default) or `V2` (deprecated).
   In `V3` mode, the `apicurioId` property identifies schema by content ID and the client, if embedding schema IDs, is assumed to use content IDs.
   In `V2` mode, the `apicurioId` property identifies schema by global ID and the client, if embedding schema IDs, is assumed to use global IDs.
