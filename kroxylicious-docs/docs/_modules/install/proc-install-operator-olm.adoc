...
[id='proc-install-operator-olm-{context}']
...
* xref:con-install-prereqs-{context}[Deployment prerequisites].
...
. Use the proxy operator xref:assembly-operator-deploy-a-proxy-{context}[to deploy a proxy].
