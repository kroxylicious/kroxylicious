...
spec:
  ...
  configTemplate:
    ...
    mapperConfig:
      separator: "-"
----
* `entityTypes` is an array of entity types to which will be isolated. Allowed values are `GROUP_ID` and `TRANSACTIONAL_ID`.
* `mapper` is the name of the naming mapping service implementation.  Currently, this must be `PrincipalEntityNameMapperService`.
* `separator` is separator character that mapper will use to separate the principal from the resource name.
