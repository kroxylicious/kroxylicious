...
Both line and block comments are supported.  Line comments are proceeded by `//`.
...
Rules must be ordered so that any `deny` rules proceed the `allow` rules.
...
This stipulation means that all rules files are have deny-by-default semantics.
