...
|`kroxylicious_client_to_proxy_idle_disconnects_total`
|Counter
|`virtual_cluster`, `node_id`
|Incremented by one every time a client connection is closed due to exceeding the configured idle timeout. +
 Requires idle timeouts to be configured via `network.proxy.unAuthenticatedIdleTimeout` or `network.proxy.authenticatedIdleTimeout`.
|===
