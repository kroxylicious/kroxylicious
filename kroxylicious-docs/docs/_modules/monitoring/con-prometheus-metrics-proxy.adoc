...
existing connections before stopping. The time spent draining is recorded by
`kroxylicious_virtual_cluster_state_duration_seconds` with `state="draining"` (see
<<Virtual cluster lifecycle metrics>>). A high drain duration typically reflects
in-flight requests waiting for broker acknowledgement; inspect broker health if
drains are consistently slow before considering tuning the drain timeout. When the
timeout expires, remaining connections are force-closed and recorded in
...
When a filter announces a client authentication outcome through the `FilterContext` API, the
proxy records it in the `kroxylicious_client_auth_total` counter. This metric is independent of
the specific filter or authentication approach used (SASL termination, SASL inspection, transport
...
|Incremented by one every time a filter announces an authentication outcome.
 The `mechanism` label identifies the SASL mechanism (for example, `SCRAM-SHA-256` or `OAUTHBEARER`).
 When the mechanism is not known, the label value is `unknown`.
