...
When a filter announces a client authentication outcome, the proxy records it
so that operators can monitor authentication activity across all virtual
...
|Incremented by one every time a filter announces a client authentication
 outcome. The `mechanism` label identifies the SASL mechanism used (for
 example `SCRAM-SHA-256`, `OAUTHBEARER`), or `unknown` if the mechanism
 could not be determined. The `outcome` label is `success` or `failure`.
