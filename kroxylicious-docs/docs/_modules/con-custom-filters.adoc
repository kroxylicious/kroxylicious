...
public class FetchFilter implements FetchRequestFilter {
    ...
    public CompletionStage<RequestFilterResult> onFetchRequest(ApiKeys apiKey,
                                                               ...
                                                               FetchRequestData request,
                                                               FilterContext context) {
        // We construct a header object for the asynchronous request.  It is important to specify the API version of the request that is to be used.  The version chosen must be a version known to the Kafka Client used by Kroxylicious and must be an API version supported by the Target Cluster.
        var metadataRequestHeader = new RequestHeaderData().setRequestApiVersion(METADATA_VERSION_SUPPORTING_TOPIC_IDS);
        // We construct a new request object. When constructing the request object, care needs to be taken to ensure the request is populated with the structure which matches the API version you have chosen.  Refer to the {kafka-protocol}[Kafka Protocol Guide] for more details.
        ...
        // We asynchronously send the request towards the Cluster and obtain a CompletionStage which will contain the response.
        var stage = context.sendRequest(metadataRequestHeader, metadataRequest);
        // We use a computation stage to mutate the filtered fetch request using the response from the request sent above
        return stage.thenApply(metadataResponse -> mutateFetchRequest(metadataResponse, request))
                     // We use another computation stage to forward the mutated request.
