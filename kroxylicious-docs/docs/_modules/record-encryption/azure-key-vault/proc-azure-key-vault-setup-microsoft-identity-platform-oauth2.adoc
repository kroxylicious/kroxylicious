...
[source,shell]
----
az ad sp create-for-rbac --name "kroxylicious" --query '[appId, password, tenant]' --output tsv
----
+
This command creates the Service Principal and outputs the Client ID, Client Secret, and Tenant ID, in that order. This example uses `kroxylicious` as the username, but you can substitute a different name if necessary.
...
az role assignment create --assignee "kroxylicious" --role "Key Vault Crypto Service Encryption User" --scope ${SCOPE_ID}
----
+
