...
----
+
This command creates the Service Principal and outputs the Client ID, Client Secret, Tenant ID, and App ID of the Service Principal, in that order. You will need the Principal ID for the next step. This example uses `kroxylicious` as the username, but you can substitute a different name if necessary.
...
----
<1> This is the object identifier (Service Principal App ID) we output in the previous step.
...
----
<1> This is the object identifier (Service Principal App ID) we output in the first step.
+
