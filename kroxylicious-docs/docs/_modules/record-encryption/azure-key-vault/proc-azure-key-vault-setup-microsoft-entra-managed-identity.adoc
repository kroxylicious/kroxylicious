...
az <RESOURCE_TYPE> identity assign --name ${HOST_RESOURCE_NAME} --resource-group ${HOST_RESOURCE_GROUP} #<2>
----
...
az role assignment create --assignee ${HOST_RESOURCE_NAME} --role "Key Vault Crypto Service Encryption User" --scope ${SCOPE_ID}
----
<1> This value should be the name of the resource group in which your key vault is deployed, which may not necessarily be the same resource group as the Kroxylicious host resource.
+
