...
[role="_abstract"]
This procedure describes how to create a `key` with `wrapKey` and `unwrapKey` operations enabled for use with the Record Encryption filter. The process uses the Azure CLI to create a key and optionally apply a rotation policy.

.Prerequisites

* Access to the Azure CLI.
* A user with the _Key Vault Crypto Officer_ role (or otherwise sufficient RBAC permissions to manage keys in Azure Key Vault).

.Procedure

. Establish a naming convention for keys within key vault for easy identification and use with the Record Encryption filter. Always use separate key vaults per application, per environment, per region; this keeps the filter’s keys separate from those used by other systems.
...
. (Optional) Configure a key rotation policy
+
When the key is created, it is recommended to use a key rotation policy.

.. Save your policy to a JSON file. An example JSON file can be found in the Azure CLI docs.
