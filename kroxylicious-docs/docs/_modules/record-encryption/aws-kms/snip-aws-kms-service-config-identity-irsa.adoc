...
** `webIdentityCredentials` configures the KMS to authenticate using a Kubernetes service-account OIDC token via STS `AssumeRoleWithWebIdentity`. All sub-fields are optional and default to the values injected by the EKS pod-identity webhook, so on a properly-annotated pod the entire block can be written as `webIdentityCredentials: {}`.
