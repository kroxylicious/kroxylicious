...
** `vaultTransitEngineUrl` xref:con-vault-setup-{context}[Vault Transit Engine URL] including the protocol part, such as `https:` or `http:`.
** `tls` (Optional) TLS trust configuration.
** `vaultToken` File containing the Vault Token.
