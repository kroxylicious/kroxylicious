...
* `openShiftRoute` exposes the virtual cluster to applications running outside the Kubernetes cluster by using OpenShift Routes.

The `clusterIP` ingress types support both TCP (plain) and TLS connections.
The `loadBalancer` and `openShiftRoute` types exclusively supports TLS.
...
IMPORTANT: OpenShift Routes are an OpenShift-only feature and are not supported on vanilla Kubernetes environments. The Kroxylicious Operator actively detects whether the OpenShift Route API is supported on the Kubernetes Server; if the feature is not available, the operator will gracefully reject any attempt by the user to use it.
...
To accomplish this, the following are automatically generated:

* A unique `bootstrapAddress`. This is the address that the clients initially use to connect to the virtual cluster.
* An `advertisedBrokerAddressPattern` that generates unique broker addresses which clients use to connect to individual brokers.

The `advertisedBrokerAddressPattern` contains the token `$(nodeId)`.
The proxy replaces this token with the broker's node ID.
This ensures that client connections are correctly routed to the intended broker.

Both `bootstrapAddress` and `advertisedBrokerAddressPattern` contain the token `$(virtualClusterName)`.
The proxy replaces this token with the virtual cluster's name.
This token is necessary when the `KafkaProxyIngress` is being shared by many virtual clusters.

Both `bootstrapAddress` and `advertisedBrokerAddressPattern` also contain the domain assigned to their associated OpenShift Route.

.`bootstrapAddress` and `advertisedBrokerAddressPattern` scheme
|===
|Address Type|Address

|`bootstrapAddress`
|`$(virtualClusterName)-bootstrap.your-domain:443`

|`advertisedBrokerAddressPattern`
|`$(virtualClusterName)-$(nodeId).your-domain:443`
|===
...
metadata:
  namespace: my-proxy
  name: openshift-route-ingress
spec:
  proxyRef:
    name: simple
  openShiftRoute: {} # <1>
----
<1> Exposes the proxy to off-cluster clients using OpenShift Routes.
