...
Assuming a `bootstrapAddress` of `$(virtualClusterName)-bootstrap.your-domain:443`, an `advertisedBrokerAddressPattern` of `$(virtualClusterName)-$(nodeId).your-domain:443`, a Kafka cluster using node IDs (0-2), and a virtual cluster name of `my-cluster`, the following SANs must be listed in the certificate:
+
[source,text]
----
mycluster-bootstrap.your-domain
mycluster-0.your-domain
mycluster-1.your-domain
mycluster-2.your-domain
----
