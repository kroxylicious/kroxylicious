...
1. A transport subject builder creates a subject with an identity (for example, from a client TLS certificate).
2. A SASL inspection or termination filter has invoked `io.kroxylicious.proxy.filter.FilterContext.clientSaslAuthenticationSuccess`.
