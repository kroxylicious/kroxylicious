...
* **Unauthenticated timeout** (`unAuthenticatedIdleTimeout`) - Applies to connections that have not completed SASL authentication. This timeout begins when the connection is established and ends when authentication completes.
* **Authenticated timeout** (`authenticatedIdleTimeout`) - Applies to connections after SASL authentication has completed. This timeout is active for the remainder of the connection's lifetime.
