...
** `bootstrapServerSelection.strategy` defines the strategy used for selecting an address from `bootstrapServers` when the proxy receives a client connection. Options are `round-robin` (default) or `random`.
