...
The cluster definition defines three properties:
...
2. `bootstrapServerSelection`: Optional strategy for selecting a bootstrap server when multiple servers are configured. Defaults to `round-robin` if not specified.
