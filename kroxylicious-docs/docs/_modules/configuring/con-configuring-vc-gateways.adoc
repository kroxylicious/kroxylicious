...
* `portIdentifiesNode` configures the gateway to use port identifies mode.
...
virtualClusters:
  - name: my-cluster-proxy
    ...
    - name: mygateway
      portIdentifiesNode:
        bootstrapAddress: localhost:9192
        advertisedBrokerAddressPattern: mycluster.example.com
        brokerStartPort: 9200
    # ...
# ...
----
where:
...
* `portIdentifiesNode` configures the gateway to use port identifies mode.
** `bootstrapAddress` provides the bootstrap address used by Kafka clients.
** `advertisedBrokerAddressPattern` provides the advertised broker address pattern used to form broker addresses. If not defined, it defaults to the hostname part of the bootstrap address.
** `brokerStartPort` provides the starting number for the broker port range. Defaults to the port of the bootstrap address plus 1.
...
** `bootstrapAddress` provides the bootstrap address used by Kafka clients.
** `advertisedBrokerAddressPattern` provides the advertised broker address pattern used to form broker addresses. If not defined, it defaults to the hostname part of the bootstrap address.
** `nodeIdRanges` defines one or more node id ranges.  If omitted, it defaults to a single range of node IDs 0..2 (inclusive).
...
If included, `$(nodeId)` is replaced by the broker's link:{ApacheKafkaDocsBase}configuration/broker-configs/#brokerconfigs_node.id[`node.id`] (or link:{ApacheKafkaDocsBase}configuration/broker-configs/#brokerconfigs_broker.id[`broker.id`]) in the target cluster.
  ...
  - name: my-cluster-proxy
    ...
    - name: mygateway
      sniHostIdentifiesNode:
        bootstrapAddress: mycluster.example.com:9192
        advertisedBrokerAddressPattern: mybroker-$(nodeId).mycluster.example.com
      tls:
         key: ...
    # ...
# ...
