...
Files providing TLS configuration are monitored and re-read when changes occur, including certificate rotation.
...
** `passwordFile` `passwordFile` refers to a file containing the password for the key.
    ...
    - name: mygateway
      ...
      tls:
        ...
        trust:
          ...
          storePassword:
            passwordFile: <path>/trust.password
          storeType: PKCS12
          trustOptions:
            clientAuth: REQUIRED
# ...
