...
jwsSignatureValidationConfig:
    ...
    algorithmConstraintType: PERMIT                               # <2>
    algorithms:                                                   # <3>
      - ES256
      - RS256
    jwsRecordHeaderKey: kroxylicious.io/jws                       # <4>
    isContentDetached: true                                       # <5>
----
<1> Set of https://datatracker.ietf.org/doc/html/rfc7517[JSON Web Keys (JWK)] to use for verifying JWS Signatures.
<2> `BLOCK` only / `PERMIT` only the specified algorithms. The default is `BLOCK`.
<3> https://datatracker.ietf.org/doc/html/rfc7518#section-3.1[JSON Web Algorithms (JWA)] to `BLOCK` / `PERMIT`. The default is `[]`.
<4> Key of the Kafka record header containing the JWS. The default is `kroxylicious.io/jws`.
