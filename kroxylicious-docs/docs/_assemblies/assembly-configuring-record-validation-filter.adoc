...
jwsSignatureValidation:
    ...
    algorithms:                                                   # <2>
      allowed:
          - ES256
          - RS256
    jwsRecordHeaderKey: kroxylicious.io/jws                       # <3>
    isContentDetached: true                                       # <4>
----
...
<4> If `true`, the validator decodes the buffer into a UTF-8 string and uses that as the JWS Payload (see https://datatracker.ietf.org/doc/html/rfc7515#appendix-F[RFC 7515 Appendix F (Detached Content)]). The default is `false`.
