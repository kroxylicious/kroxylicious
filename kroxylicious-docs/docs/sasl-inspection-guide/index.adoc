...
If the client's authentication with the broker is successful, the filter uses the SASL subject builder to construct a subject containing the authenticated principal.
...
If an attempt is made to use a unsupported token type, the authentication will fail with a SASL error.
