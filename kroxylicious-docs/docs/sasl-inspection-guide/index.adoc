...
It makes the authenticated principal available to the other filters in the chain, so that they may know on whose behalf they are acting.
...
This is done to avoid the plain-text passwords existing tin the proxy's memory.
