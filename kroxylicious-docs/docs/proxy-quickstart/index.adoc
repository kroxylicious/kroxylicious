...
----
+
Otherwise, the official Apache Kafka® https://kafka.apache.org/quickstart[Quickstart] provides further
...
For this Quickstart, we'll use the example configuration file located at `+config/example-proxy-config.yaml+`. This file is pre-configured for a local Kafka cluster running on default ports.
