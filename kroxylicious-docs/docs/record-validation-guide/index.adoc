...
3. JWS Signature Validation ensures the configured Kafka header contains a valid https://datatracker.ietf.org/doc/html/rfc7515[JSON Web Signature (JWS)] Signature.
...
include::_modules/record-validation/proc-configuring-record-validation-filter.adoc[leveloffset=+1]
ifdef::include-platform-bare-metal[]
include::_modules/record-validation/con-example-proxy-config.adoc[leveloffset=+2]
endif::[]
ifdef::include-platform-kubernetes[]
include::_modules/record-validation/con-example-kafkaprotocolfilter-resource.adoc[leveloffset=+2]
endif::[]
