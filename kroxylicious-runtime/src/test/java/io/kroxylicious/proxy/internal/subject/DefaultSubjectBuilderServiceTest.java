...
public class DefaultSubjectBuilderServiceTest {

    private static YAMLMapper mapper = new YAMLMapper();

    private static void tls(DefaultSubjectBuilderService.Config builderConfig,
        ...
        var service = new DefaultSubjectBuilderService();
        service.initializeTransportSubjectBuilderService(builderConfig);
        TransportSubjectBuilder builder = service.buildTransportSubjectBuilderService();
    ...
    private static void sasl(DefaultSubjectBuilderService.Config builderConfig,
                             String authorizedId,
                             @Nullable Subject expectedSubject) {
        var service = new DefaultSubjectBuilderService();
        service.initializeTransportSubjectBuilderService(builderConfig);
        var builder = service.buildSaslSubjectBuilderService();
        ...
        return List.of(
                Arguments.argumentSet(
                        ...
                        "Unknown `from` 'thisIsNotKnown', supported values are: "
                                + "'saslAuthorizedId', 'clientTlsSubject', 'clientTlsSanRfc822Name', "
    ...
    void invalidShouldThrowDuringDeserialization(String rule, String expectedExceptionMessage) {
        assertThatThrownBy(() -> {
            mapper.readValue(rule, DefaultSubjectBuilderService.Config.class);
        })
                ...
                Arguments.argumentSet("multiple", "CN=test,OU=testing", """
                        addPrincipals:
                          ...
                          - from: clientTlsSubject
                            ...
                            principalFactory: io.kroxylicious.proxy.internal.subject.UserFactory
                        """,
                        new Subject(new User("XTESTY"), new User("xtestingy")), null),
    ...
    void rulesShouldWorkWithX500Subjects(String x500Principal, String rule, Subject expectedSubject, Exception expectedException) throws JsonProcessingException {
        ...
        DefaultSubjectBuilderService.Config builderConfig = mapper.readValue(rule, DefaultSubjectBuilderService.Config.class);
    ...
    void firstMatchingMapShouldWin()
        ...
        DefaultSubjectBuilderService.Config builderConfig = mapper.readValue("""
                addPrincipals:
                  - from: saslAuthorizedId
                    map:
                      - replaceMatch: /foo-(.*?)/$1/
                      ...
                      - else: anonymous
                    principalFactory: io.kroxylicious.proxy.internal.subject.UserFactory
                """, DefaultSubjectBuilderService.Config.class);
