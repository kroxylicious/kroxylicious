 ...
 * client with.
 */
public class VendoredKafkaProxyExceptionMapper {

    private static final int THROTTLE_TIME_MS = 0;
    private static final String USER_PRINCIPAL_TYPE = "User";
    private static final String ANONYMOUS_PRINCIPLE_NAME = "ANONYMOUS";

    private record TopicPartition(String topic, int partition) {

    }

    private record TopicIdPartition(Uuid topicId, int partition, String topic) {

    }

    private VendoredKafkaProxyExceptionMapper() {
    }
     ...
     */
    @Nullable
    static ApiMessage errorResponseData(ApiKeys apiKey, ApiMessage requestBody, short apiVersion, Errors error, @Nullable String message) {
    ...
    private static CreateDelegationTokenResponseData createDelegationTokenErrorResponse(short apiVersion, short code) {
        CreateDelegationTokenResponseData response = new CreateDelegationTokenResponseData()
                .setThrottleTimeMs(THROTTLE_TIME_MS)
                .setErrorCode(code)
                .setPrincipalType(USER_PRINCIPAL_TYPE)
                .setPrincipalName(ANONYMOUS_PRINCIPLE_NAME)
        ...
        if (apiVersion > 2) {
            response.setTokenRequesterPrincipalType(USER_PRINCIPAL_TYPE)
                    .setTokenRequesterPrincipalName(ANONYMOUS_PRINCIPLE_NAME);
        }
        return response;
    }
