...
public class ProxyChannelStateMachine {
     ...
     * </p>
     *
     * @param context the immutable HaProxy context extracted from the decoded message
     */
    void onHaProxyMessageReceived(HaProxyContext context) {
        kafkaSession.setHaProxyContext(context);
        if (state instanceof ProxyChannelState.ClientActive clientActive) {
            toHAProxy(clientActive.toHaProxy());
        }
        else {
