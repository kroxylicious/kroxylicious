...
* [#4769](https://github.com/kroxylicious/kroxylicious/issues/4769): changed(filter-test-support): MockFilterContextAssert.errorResponse asserts on Errors codes
* [#4771](https://github.com/kroxylicious/kroxylicious/pull/4771): remove kafka-clients ApiException from KafkaProxyExceptionMapper
...
* [#3785](https://github.com/kroxylicious/kroxylicious/issues/3785): feat(record-validation): remove deprecated `WireFormatVersion.V2` enum value.
...
* [#4768](https://github.com/kroxylicious/kroxylicious/pull/4768): fix(api)!: stop deriving TopicNameMappingException cause from Errors.exception()
* [#4808](https://github.com/kroxylicious/kroxylicious/issues/4808): fix(runtime): deliver a per-route filter's out-of-band response by recipient, so it is not lost when two or more routes issue out-of-band requests on one connection
...
* [#4769](https://github.com/kroxylicious/kroxylicious/issues/4769): `MockFilterContextAssert.errorResponse()` no longer returns a `ThrowableAssert<ApiException>`.
...
* [#4771](https://github.com/kroxylicious/kroxylicious/pull/4771): An undocumented behaviour in the Filter API has been removed. Previously if a request filter returned an exceptionally completed future, we would use that exception type to determine which error code to set on the response. We are removing all integration with the kafka exception hierarchy so now any exceptionally completed request futures will result in an UNKNOWN_SERVER_ERROR response to the client. Filters that want specialized error responses should use the `errorResponse` APIs on `FilterContext#requestFilterResultBuilder`.
* [#4801](https://github.com/kroxylicious/kroxylicious/pull/4801): BREAKING CHANGE: All filter and router implementations must be updated.
  ...
  This also partially upgrades the project to Apache Kafka 4.3.
...
* [#3785](https://github.com/kroxylicious/kroxylicious/issues/3785): The deprecated `V2` value of the record validation filter's `wireFormatVersion` configuration option is removed.
...
* [#4744](https://github.com/kroxylicious/kroxylicious/issues/4744): The `wireFormatVersion` option of the record validation filter's `schemaValidationConfig` is removed.
...
* [#4768](https://github.com/kroxylicious/kroxylicious/pull/4768): Small breaking change for filter authors: the `TopicNameMappingException(Errors)` and
