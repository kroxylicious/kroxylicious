...
You'll need the following a developer machine with the following software installed.
...
apply the Record Encryption filter to encrypt as they are sent by producers and decrypt them as they get fetched by consumers.
...
Ignore the warning about the UNKNOWN_TOPIC_OR_PARTITION, the topic will be created automatically.
...
You want to see that the records are truly encrypted on the broker, right.  Let's use the console consumer again,
