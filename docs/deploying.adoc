...
The `PortPerBroker` scheme can be used with either plain or TLS downstream connections.
...
brokers.   As this relies on SNI, the use of <<Downstream TLS>> is *required*.
...
The following illustrates enabling TLS using platform trust:
