...
virtualClusters:
  demo:                                         # <1>
    ...
    clusterNetworkAddressConfigProvider:
      type: PortPerBrokerContributor                       # <3>
      config:
...
<3> The name of a cluster network address config provider. The built-in types are `PortPerBrokerContributor` and `SniRoutingContributor`.
