...
Message metrics allow the number of {kafka-protocol}#protocol_messages[Kafka protocol messages] that flow through the proxy to be understood.
...
If a connection error occurs at the client, before the virtual cluster has been established, the error `kroxylicious_client_to_proxy_errors_total` with the  virtual cluster and node ID labels set to the empty string`""`.
...
When the client connects to boostrap endpoint of the virtual cluster, metrics are recorded with a node ID value of `bootstrap`.

NOTE: There are several situations where metric value recorded at the client end will exceed the value recorded at the broker end.
