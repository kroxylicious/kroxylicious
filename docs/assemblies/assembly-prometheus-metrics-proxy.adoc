...
There are two _client end_ counters counting requests as they arrive from the client (`kroxylicious_client_to_proxy_request_total`) and responses as they are returned to the client (`kroxylicious_proxy_to_client_response_total`).
There are two _broker end_ counters counting requests as they go to the broker (`kroxylicious_proxy_to_server_request_total`) and responses as they returned by the broker (`kroxylicious_proxy_to_server_response_total`).
...
NOTE: There are several situations where the metric value recorded by the client end counters (i.e. `kroxylicious_client_to_proxy_&#42;_total`) will exceed the value recorded by the broker end counters (i.e `kroxylicious_proxy_to_server_&#42;_total`).
This occurs when a filter chooses to send a short-circuit response, or chooses to close the connection.
It can also occur in some error situations where connectivity to the upstream cluster is lost as a message transits the proxy.
