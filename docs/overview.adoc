...
First let's define the concepts in the landscape surrounding Kroylicious.
...
reverse order, each having the opportunity to inspecting and/or manipulating the response. Eventually a response is
...
information so that they reflect the broker addresses exposed by the Virtual Cluster. These are the `Metadata`,
`DescribeCluster` and `FindCoordinator` responses. _Filter authors_ are free to write their own filters that
