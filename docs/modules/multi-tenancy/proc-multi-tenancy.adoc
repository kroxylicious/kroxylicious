...
By default, the prefix representing a tenant is taken from the name of the virtual cluster representing the tenant. 
For example, if the virtual cluster is named `tenant-1`, the prefix is `tenant-1`.
Alternatively, you can configure a custom prefix that overrides the virtual cluster name.
...
filters:
  - type: MultiTenant
    config:
      ...
      prefixResourceName: "tenant-a" # <2>
----
<1> The separator used for the prefix. 
If a separator is not specified, `-` is the default.
<2> Optional custom prefix for resource names. If specified, it overrides the virtual cluster name as the prefix. If not specified, the virtual cluster name is used (default behavior).
+
