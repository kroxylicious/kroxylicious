...
The `advertisedBrokerAddressPattern` configuration parameter accepts the `$(virtualClusterName)` replacement token, which is optional.
If included, `$(virtualClusterName)` is replaced with the name of the gateway's virtual cluster.

The `bootstrapAddress` configuration parameter accepts the `$(virtualClusterName)` replacement token, which is optional.
If included, `$(virtualClusterName)` is replaced with the name of the gateway's virtual cluster.
