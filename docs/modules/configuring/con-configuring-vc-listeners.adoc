...
node ids `1..3` and `101..103`.   The advertised broker address is defined as `mycluster.example.com`.  Port
...
In the SNI Host Identifies Node scheme, unique broker host names are used to know where to route the traffic.
...
With the example configuration above, the listener accepts all traffic on port 9192. Any TLS connections arriving with
...
It exposes a target cluster with any number of brokers.  It does not need prior knowledge of the node IDs used by
the brokers.
