...
spec:
  ...
  ingresses:
    - ingressRef: # <3>
        name: cluster-ip
----
<1> The `proxyRef` names the `KafkaProxy` associated with the virtual cluster.
...
<3> Ingresses that expose the virtual cluster.
  Each ingress references a `KafkaProxyIngress`, which must be in the same namespace as the `VirtualKafkaCluster`.
  ...
  ingresses: # <3>
    ...
    - ingressRef:
        ...
        certificateRef:
          ...
          kind: Secret
----
<1> The `proxyRef` names the `KafkaProxy` associated with this virtual cluster.
It must be in the same namespace as the `VirtualKafkaCluster`.
<2> The `KafkaService` that defines the Kafka cluster proxied by the virtual cluster.
It must be in the same namespace as the `VirtualKafkaCluster`.
<3> Ingresses that expose the virtual cluster.
<4> Reference to a secret containing the server certificate for the `clusterIP` ingress.
<5> Reference to a secret containing the server certificate for the `loadBalancer` ingress.
