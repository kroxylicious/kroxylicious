...
Each virtual cluster targets a single gateway on the target cluster, so multiple gateways on the Kafka side are represented as multiple virtual clusters by the proxy.
