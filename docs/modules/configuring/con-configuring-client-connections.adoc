...
If both an allow and a deny list are specified, the resulting TLS protocols list includes only those protocols from the
...
If both an allow and a deny list are specified, the resulting cipher suite list includes only those ciphers from the
allow list that are not in the deny list.
