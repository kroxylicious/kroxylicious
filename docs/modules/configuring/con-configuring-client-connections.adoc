...
If both an allow and a deny list are specified, the result is the TLS protocols that are in allowed but not in denied.
...
If both an allow and a deny list are specified, the result is the cipher suites that are in allowed but not in denied.
