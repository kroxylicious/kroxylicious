...
<9> The listeners of the virtual cluster defining how the virtual cluster is presented to the network.
<10> The identification scheme used to route traffic to brokers - the possibilities are `sniHostIdentifiesNode` and `portIdentifiesNode`.
...
<12> TLS encryption by this listener for securing connections with the clients.
