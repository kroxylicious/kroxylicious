...
To use the filter, an administrator or an administrative process must create the encryption keys within Vault that will
be used by the xref:con-topic-encryption-overview-{context}[envelope encryption] process.
