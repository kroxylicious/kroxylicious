...
When a KEK is rotated, the new key material is eventually used for newly produced records. Existing records, encrypted with older KEK versions, remain decryptable as long as the previous KEK versions are still in the KMS.

When the KEK is rotated in the external KMS, it will take up to 2 hours (by default) before all records produced by the filter
contain a DEK encrypted with the new key material. This is because existing encrypted DEKs are used for a configurable
amount of time after creation, the Filter caches the encrypted DEK.

In a worst case scenario like the key material being compromised you can execute a rolling restart of your cluster of
Kroxylicious instances after the key rotation. After the restart, messages being encrypted by the Record Encryption Filter
will all use the latest key material.
