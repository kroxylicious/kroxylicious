...
On the produce path, the filter is encrypting records.  The filter uses a selector to determine which KEK to use. It then
...
Null record values sent by producers are passed through unencrypted.  This serves to allow tombstoning.
...
For example, if using the `TemplateKekSelector` with the template `topic.$\{topicName}`, create a key for every topic that
is to be encrypted with the key name matching the topic name, prefixed by the string `topic.`.
