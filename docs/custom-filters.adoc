...
filter implementations should minimise the occasions on which not yet completed `CompletionStage` are returned.
...
----
<1> We construct a new request object.  This will be the asynchronous request.
<2> We send the request towards the Cluster, specifying the api version to use and obtaining a CompletionStage for the
response.
<3> We use CompletionStage chaining to mutate the filtered fetch request using the asynchronous response.
