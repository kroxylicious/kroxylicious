...
public class FetchFilter implements FetchRequestFilter {
    ...
    public CompletionStage<RequestFilterResult> onFetchRequest(ApiKeys apiKey,
        ...
        return stage.thenApply(metadataResponse -> mutateFetchRequest(metadataResponse.message(), request)) // <4>
...
that is to be used.  The version chosen must be a version know to the Kafka Client used by Kroxylicious
and must be an API version supported by the Target Cluster.
<2> We construct a new request object. When constructing the request object, take care to populate  the fields known to
the request object at the API version you have chosen.  Refer to the {kafka-protocol}[Kafka Protocol Guide] for more details.
