...
class UserNamespaceFilterTest {
    ...
    private final UserNamespaceFilter filter = new UserNamespaceFilter(
            new UserNamespace.Config(Set.of(UserNamespace.ResourceType.GROUP_ID, UserNamespace.ResourceType.TRANSACTIONAL_ID)));
    ...
    @Mock(strictness = LENIENT)
    private ClientSaslContext clientSaslContext;
    ...
    void beforeEach() {
        ...
        when(context.clientSaslContext()).thenReturn(Optional.of(clientSaslContext));
        when(clientSaslContext.authorizationId()).thenReturn(TENANT_1);
    }
