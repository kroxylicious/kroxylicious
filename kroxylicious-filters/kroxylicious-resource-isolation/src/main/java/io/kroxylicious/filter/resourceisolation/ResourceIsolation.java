...
/**
 * A {@link FilterFactory} for {@link ResourceIsolationFilter}.
 */
@Plugin(configType = ResourceIsolation.Config.class)
public class ResourceIsolation implements FilterFactory<ResourceIsolation.Config, ResourceIsolation.Config> {
    ...
    @Override
    @SuppressWarnings("java:S2638") // Tightening Unknown Nullness
    public ResourceIsolationFilter createFilter(FilterFactoryContext context, @NonNull Config configuration) {
        if (mapper == null) {
            throw new IllegalStateException("filter factory has not been initialized");
        }
        return new ResourceIsolationFilter(configuration.resourceTypes, mapper);
    }
