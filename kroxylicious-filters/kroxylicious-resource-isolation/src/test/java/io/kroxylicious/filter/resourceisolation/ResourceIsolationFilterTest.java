...
class ResourceIsolationFilterTest {
    private static final Pattern TEST_RESOURCE_FILTER = Pattern.compile(
            String.format("%s/[A-Z_]+/\\d+/.*\\.yaml", ResourceIsolationFilterTest.class.getPackageName().replace(".", "/")));
    private static final String TENANT_1 = "tenant1";

    private static List<ResourceInfo> getTestResources() throws IOException {
        var resources = ClassPath.from(ResourceIsolationFilterTest.class.getClassLoader()).getResources().stream()
    ...
    private final ResourceIsolationFilter filter = new ResourceIsolationFilter(
            Set.of(ResourceIsolation.ResourceType.GROUP_ID, ResourceIsolation.ResourceType.TRANSACTIONAL_ID),
