...
additional memory for the broker process and Maven. Review the Confluent image's
license terms for your intended use.
...
```shell
./mvnw -pl :kroxylicious-mds-impersonation -am -Dquick install
./mvnw -pl :kroxylicious-mds-impersonation \
...
The `Confluent MDS integration tests` workflow performs the dependency build,
environment startup, provisioning and test command automatically. It uploads an
...
* Two alice consumers rebalance two partitions while records are produced and
  consumed through the expiry of short-lived MDS tokens and reconnection.
* The mock-wire test separately checks that upstream SASL authentication precedes
  every forwarded client request and that MDS receives the proxy certificate.
