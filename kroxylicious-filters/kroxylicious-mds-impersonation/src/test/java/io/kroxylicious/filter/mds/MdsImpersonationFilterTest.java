...
class MdsImpersonationFilterTest {
    ...
    void holdsClientTrafficUntilTokenAndUpstreamAuthenticationComplete() {
        ...
        assertThat(context.sendRequestInvocations()).hasSize(2);
        assertThat(context.sendRequestInvocations().get(0).request().apiKey()).isEqualTo(ApiKeys.SASL_HANDSHAKE.id);
        var auth = (SaslAuthenticateRequestData) context.sendRequestInvocations().get(1).request();
    ...
    void honorsBrokerSessionDeadlineBeforeJwtExpiry() {
        ...
        var context = context("alice").withSendRequestResponseEnqueued(new SaslAuthenticateResponseData().setSessionLifetimeMs(30000)).build();
        context = runtimeResults(context);
        filter.onRequest(ApiKeys.API_VERSIONS, (short) 3, header, request, context).toCompletableFuture().join();
