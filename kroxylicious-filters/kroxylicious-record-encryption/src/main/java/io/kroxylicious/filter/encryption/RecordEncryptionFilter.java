...
public class RecordEncryptionFilter<K> implements ProduceRequestFilter, FetchResponseFilter, ShareFetchResponseFilter {
    ...
    RecordEncryptionFilter(EncryptionManager<K> encryptionManager, DecryptionManager decryptionManager, TopicNameBasedKekSelector<K> kekSelector,
                           @NonNull FilterThreadExecutor filterThreadExecutor, UnresolvedKeyPolicy unresolvedKeyPolicy) {
    ...
    public CompletionStage<RequestFilterResult> onProduceRequest(short apiVersion, RequestHeaderData header, ProduceRequestData request, FilterContext context) {
        return withTopicNameMapping(request.topicData().stream().map(TopicProduceData::topicId), context,
                topicNameMapping -> maybeEncrypt(header, request, context, topicNameMapping));
    }

    @NonNull
    private CompletionStage<RequestFilterResult> maybeEncrypt(RequestHeaderData header, ProduceRequestData request, FilterContext context,
                                                              TopicNameMapping topicNameMapping) {
        if (topicNameMapping.anyFailures()) {
            return maybeRespondWithUnknownTopicId(header, request, context, topicNameMapping);
        }
        else {
    ...
    private static <T> CompletionStage<T> withTopicNameMapping(Stream<Uuid> topicIds, FilterContext context, Function<TopicNameMapping, CompletionStage<T>> mapping) {
        List<Uuid> filteredIds = topicIds.filter(uuid -> !Uuid.ZERO_UUID.equals(uuid)).toList();
        return context.topicNames(filteredIds).thenCompose(mapping);
    }
    ...
    private CompletionStage<ProduceRequestData> maybeEncodeProduce(ProduceRequestData request, FilterContext context, TopicNameMapping topicNameMapping) {
        ...
        var topicNameToData = request.topicData().stream()
                .collect(Collectors.toMap(topicProduceData -> getName(topicProduceData, topicNameMapping), Function.identity()));
                ...
                .thenCompose(kekSelection -> {
                    ...
                    var futures = kekSelection.topicNameToKekId().entrySet().stream().flatMap(e -> {
                        ...
                        return tpd.partitionData().stream().map(ppd -> {
                            ...
                            return encryptionManager.encrypt(topicName, ppd.index(), new EncryptionScheme<>(kekId, EnumSet.of(RecordField.RECORD_VALUE)), records,
                                    context::createByteBufferOutputStream).thenApply(ppd::setRecords).thenApply(produceData -> {
                                        encryptedRecordsTotal.withTags(RecordEncryptionMetrics.TOPIC_NAME, topicName)
    ...
    private static String getName(TopicProduceData topicProduceData, TopicNameMapping topicNameMapping) {
        if (topicProduceData.name() != null && !topicProduceData.name().isEmpty()) {
            return topicProduceData.name();
        }
        else {
            Map<Uuid, String> names = topicNameMapping.topicNames();
            Uuid topicId = topicProduceData.topicId();
            if (names.containsKey(topicId)) {
                return names.get(topicId);
            }
            else {
                throw new IllegalStateException("no topic name in mapping for " + topicId);
            }
        }
    }
    ...
    private void generatePlainRecordsMetrics(Meter.MeterProvider<Counter> plainRecordsTotal, Set<String> unresolvedTopicNames,
        ...
        topicNameToData.entrySet().stream().filter(topicDataEntry -> unresolvedTopicNames.contains(topicDataEntry.getKey())).forEach(
                topicData -> topicData.getValue().partitionData().forEach(
                        produceData -> plainRecordsTotal.withTags(RecordEncryptionMetrics.TOPIC_NAME, topicData.getKey())
                                .increment(RecordEncryptionUtil.totalRecordsInBatches((MemoryRecords) produceData.records()))));
    }
    ...
    public CompletionStage<ResponseFilterResult> onShareFetchResponse(short apiVersion, ResponseHeaderData header, ShareFetchResponseData response,
        ...
        return withTopicNameMapping(response.responses().stream().map(ShareFetchableTopicResponse::topicId), context,
                topicNameMapping -> maybeDecodeShareFetch(topicNameMapping, response.responses(), context).thenCompose(topicResponses -> {
    ...
    public CompletionStage<ResponseFilterResult> onFetchResponse(short apiVersion, ResponseHeaderData header, FetchResponseData response, FilterContext context) {
        return withTopicNameMapping(response.responses().stream().map(FetchableTopicResponse::topicId), context,
                topicNameMapping -> maybeDecodeFetch(header, response, context, topicNameMapping));
    }

    @NonNull
    private CompletionStage<ResponseFilterResult> maybeDecodeFetch(ResponseHeaderData header, FetchResponseData response, FilterContext context,
                                                                   TopicNameMapping topicNameMapping) {
        return maybeDecodeFetch(response.responses(), context, topicNameMapping).thenCompose(
                responses -> context.forwardResponse(header, response.setResponses(responses)))
                .exceptionallyCompose(throwable -> {
                    if (throwable.getCause() instanceof UnknownKeyException) {
                        // #maybeDecodePartitions will have set the RESOURCE_NOT_FOUND error code on the partition(s) that failed to decrypt
                        // and will have logged the affected topic-partitions.
        ...
        for (ShareFetchableTopicResponse topicData : topics) {
            ...
            if (topicName != null) {
                result.add(maybeDecodePartitions(topicName, partitions, context, ShareFetchResponseData.PartitionData::records,
                        ShareFetchResponseData.PartitionData::partitionIndex, partitionData -> partitionData::setRecords,
                        ShareFetchResponseData.PartitionData::setErrorCode).thenApply(kk -> {
    ...
    private static CompletableFuture<ShareFetchableTopicResponse> setAllPartitionsToError(ShareFetchableTopicResponse topicData, TopicNameMappingException failure) {
    ...
    private CompletionStage<List<FetchableTopicResponse>> maybeDecodeFetch(List<FetchableTopicResponse> topics, FilterContext context,
        ...
        for (FetchableTopicResponse topicData : topics) {
            String topicName = getTopicName(topicData, topicNameMapping);
            if (topicName == null) {
                topicData.partitions().forEach(partitionData -> {
                    partitionData.setErrorCode(Errors.UNKNOWN_TOPIC_ID.code());
                    partitionData.setRecords(MemoryRecords.EMPTY);
                    partitionData.setHighWatermark(-1);
                });
                result.add(CompletableFuture.completedFuture(topicData));
            }
            else {
                ...
                result.add(maybeDecodePartitions(topicName, partitions, context, PartitionData::records, PartitionData::partitionIndex,
                        partitionData -> partitionData::setRecords, PartitionData::setErrorCode).thenApply(kk -> {
                            topicData.setPartitions(kk);
                            return topicData;
                        }));
            }
    ...
    private static @Nullable String getTopicName(FetchableTopicResponse topicData, TopicNameMapping topicNameMapping) {
    ...
    private <T> CompletionStage<T> maybeDecodeRecords(String topicName, MemoryRecords memoryRecords, FilterContext context, int partition,
                                                      Function<MemoryRecords, T> setRecords) {
        return decryptionManager.decrypt(topicName, partition, memoryRecords, context::createByteBufferOutputStream).thenApply(setRecords);
    }
