...
<#macro mapRequestFields messageSpec fieldVar children indent>
    ...
    <#list children>
        ...
        ${pad}if (shouldMap.apply(EntityIsolation.ResourceType.${field.entityType}) && <@inVersionRange "header.requestApiVersion()", messageSpec.validVersions.intersect(field.versions)/>) {
...
${pad}// process entity fields defined at this level
        <#items as field>
            <#local getter="${field.name?uncap_first}" setter="set${field.name}" />
            <#if filteredEntityTypes?seq_contains(field.entityType)>
${pad}if (shouldMap.apply(EntityIsolation.ResourceType.${field.entityType}) && <@inVersionRange "apiVersion", messageSpec.validVersions.intersect(field.versions)/> && ${fieldVar}.${getter}() != null) {
                <#if field.type == 'string'>
...
class ${messageSpecPair.name}EntityIsolationProcessor implements EntityIsolationProcessor<${messageSpecPair.request.dataClassName}, ${messageSpecPair.response.dataClassName}, Void> {
    ...
    ${messageSpecPair.name}EntityIsolationProcessor(Function<EntityIsolation.ResourceType, Boolean> shouldMap, EntityNameMapper mapper) {
