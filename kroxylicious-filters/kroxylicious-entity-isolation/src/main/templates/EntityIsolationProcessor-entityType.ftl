...
<#macro mapRequestFields messageSpec fieldVar children indent>
    ...
    <#list children>
        ...
        ${pad}if (shouldMap.test(EntityType.${field.entityType}) && <@inVersionRange "header.requestApiVersion()", messageSpec.validVersions.intersect(field.versions)/> && ${fieldVar}.${getter}() != null) {
            ...
            ${pad}    ${fieldVar}.${setter}(${fieldVar}.${getter}().stream().map(orig -> mapper.map(mapperContext, EntityType.${field.entityType}, orig)).toList());
                <#else>
...
/**
* Entity isolation processor for ${inputSpec.kafkaApiKeyEnumName}.
*/
...
<#if inputSpec.request.hasAtLeastOneEntityField(filteredEntityTypes)>
    <#assign key="${inputSpec.kafkaApiKeyEnumName}" dataClass="${inputSpec.request.dataClassName}"/>
    @Override
    public boolean shouldHandleRequest(short apiVersion) {
        return <@inVersionRange "apiVersion" inputSpec.request.intersectedVersionsForEntityFields(filteredEntityTypes)/>;
    }
...
<#if inputSpec.response.hasAtLeastOneEntityField(filteredEntityTypes)>
    <#assign key="${inputSpec.kafkaApiKeyEnumName}" dataClass="${inputSpec.response.dataClassName}"/>
    @Override
    public boolean shouldHandleResponse(short apiVersion) {
        return <@inVersionRange "apiVersion" inputSpec.response.intersectedVersionsForEntityFields(filteredEntityTypes)/>;
    }
