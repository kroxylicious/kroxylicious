...
<#macro mapRequestFields messageSpec fieldVar children indent>
    ...
    <#list children>
        ...
        ${pad}            .filter(e -> shouldMap.apply(e.getValue().get())).forEach(e -> {
...
<#if messageSpecPair.request.hasResourceList>
    ...
    public boolean shouldHandleRequest(short apiVersion) {
        return <@inVersionRange "apiVersion" messageSpecPair.request.intersectedVersionsForResourceList()/>;
    }
...
<#if messageSpecPair.response.hasResourceList>
    ...
    public boolean shouldHandleResponse(short apiVersion) {
        return <@inVersionRange "apiVersion" messageSpecPair.response.intersectedVersionsForResourceList()/>;
    }
