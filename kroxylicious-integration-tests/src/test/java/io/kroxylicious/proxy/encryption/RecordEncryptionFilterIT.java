...
class RecordEncryptionFilterIT {
    ...
    void userCanChooseToRejectRecordsWhichWeCannotResolveKeysFor(KafkaCluster cluster, Topic encryptedTopic, Topic plainTopic, TestKmsFacade<?, ?, ?> testKmsFacade) {
        ...
        try (var tester = kroxyliciousTester(builder);
            ...
            producer.flush();
            assertThat(sendA).failsWithin(5, TimeUnit.SECONDS).withThrowableThat().withCause(new InvalidRecordException("failed to resolve key for: [" + plainTopic.name() + "]"));
            assertThat(sendB).failsWithin(5, TimeUnit.SECONDS).withThrowableThat().withCause(new InvalidRecordException("failed to resolve key for: [" + plainTopic.name() + "]"));
        }
    }
