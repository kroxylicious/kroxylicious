...
/**
 * A filter that counts the number of times {@code onResponse} is invoked per API key, using static
 * state keyed by a caller-supplied ID so that the test and the filter instance running inside the
 * proxy can share data without a network boundary.
 */
public class ResponseCountingFilter implements ResponseFilter {
    ...
    ResponseCountingFilter(String counterId) {
