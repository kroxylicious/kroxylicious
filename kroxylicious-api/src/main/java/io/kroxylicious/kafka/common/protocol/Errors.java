...
import io.kroxylicious.kafka.common.errors.BrokerIdNotRegisteredException;
import io.kroxylicious.kafka.common.errors.BrokerNotAvailableException;
import io.kroxylicious.kafka.common.errors.ClusterAuthorizationException;
import io.kroxylicious.kafka.common.errors.ConcurrentTransactionsException;
import io.kroxylicious.kafka.common.errors.ControllerMovedException;
import io.kroxylicious.kafka.common.errors.CoordinatorLoadInProgressException;
import io.kroxylicious.kafka.common.errors.CoordinatorNotAvailableException;
import io.kroxylicious.kafka.common.errors.CorruptRecordException;
import io.kroxylicious.kafka.common.errors.DelegationTokenAuthorizationException;
import io.kroxylicious.kafka.common.errors.DelegationTokenDisabledException;
import io.kroxylicious.kafka.common.errors.DelegationTokenExpiredException;
import io.kroxylicious.kafka.common.errors.DelegationTokenNotFoundException;
import io.kroxylicious.kafka.common.errors.DelegationTokenOwnerMismatchException;
import io.kroxylicious.kafka.common.errors.DuplicateBrokerRegistrationException;
import io.kroxylicious.kafka.common.errors.DuplicateResourceException;
import io.kroxylicious.kafka.common.errors.DuplicateSequenceException;
import io.kroxylicious.kafka.common.errors.DuplicateVoterException;
import io.kroxylicious.kafka.common.errors.ElectionNotNeededException;
import io.kroxylicious.kafka.common.errors.EligibleLeadersNotAvailableException;
import io.kroxylicious.kafka.common.errors.FeatureUpdateFailedException;
import io.kroxylicious.kafka.common.errors.FencedInstanceIdException;
import io.kroxylicious.kafka.common.errors.FencedLeaderEpochException;
import io.kroxylicious.kafka.common.errors.FencedMemberEpochException;
import io.kroxylicious.kafka.common.errors.FencedStateEpochException;
import io.kroxylicious.kafka.common.errors.FetchSessionIdNotFoundException;
import io.kroxylicious.kafka.common.errors.FetchSessionTopicIdException;
import io.kroxylicious.kafka.common.errors.GroupAuthorizationException;
import io.kroxylicious.kafka.common.errors.GroupIdNotFoundException;
import io.kroxylicious.kafka.common.errors.GroupMaxSizeReachedException;
import io.kroxylicious.kafka.common.errors.GroupNotEmptyException;
import io.kroxylicious.kafka.common.errors.GroupSubscribedToTopicException;
import io.kroxylicious.kafka.common.errors.IllegalGenerationException;
import io.kroxylicious.kafka.common.errors.IllegalSaslStateException;
import io.kroxylicious.kafka.common.errors.InconsistentClusterIdException;
import io.kroxylicious.kafka.common.errors.InconsistentGroupProtocolException;
import io.kroxylicious.kafka.common.errors.InconsistentTopicIdException;
import io.kroxylicious.kafka.common.errors.InconsistentVoterSetException;
import io.kroxylicious.kafka.common.errors.IneligibleReplicaException;
import io.kroxylicious.kafka.common.errors.InvalidCommitOffsetSizeException;
import io.kroxylicious.kafka.common.errors.InvalidConfigurationException;
import io.kroxylicious.kafka.common.errors.InvalidFetchSessionEpochException;
import io.kroxylicious.kafka.common.errors.InvalidFetchSizeException;
import io.kroxylicious.kafka.common.errors.InvalidGroupIdException;
import io.kroxylicious.kafka.common.errors.InvalidPartitionsException;
import io.kroxylicious.kafka.common.errors.InvalidPidMappingException;
import io.kroxylicious.kafka.common.errors.InvalidPrincipalTypeException;
import io.kroxylicious.kafka.common.errors.InvalidProducerEpochException;
import io.kroxylicious.kafka.common.errors.InvalidRecordStateException;
import io.kroxylicious.kafka.common.errors.InvalidRegistrationException;
import io.kroxylicious.kafka.common.errors.InvalidRegularExpression;
import io.kroxylicious.kafka.common.errors.InvalidReplicaAssignmentException;
import io.kroxylicious.kafka.common.errors.InvalidReplicationFactorException;
import io.kroxylicious.kafka.common.errors.InvalidRequestException;
import io.kroxylicious.kafka.common.errors.InvalidRequiredAcksException;
import io.kroxylicious.kafka.common.errors.InvalidSessionTimeoutException;
import io.kroxylicious.kafka.common.errors.InvalidShareSessionEpochException;
import io.kroxylicious.kafka.common.errors.InvalidTimestampException;
import io.kroxylicious.kafka.common.errors.InvalidTopicException;
import io.kroxylicious.kafka.common.errors.InvalidTxnStateException;
import io.kroxylicious.kafka.common.errors.InvalidTxnTimeoutException;
import io.kroxylicious.kafka.common.errors.InvalidUpdateVersionException;
import io.kroxylicious.kafka.common.errors.InvalidVoterKeyException;
import io.kroxylicious.kafka.common.errors.KafkaStorageException;
import io.kroxylicious.kafka.common.errors.LeaderNotAvailableException;
import io.kroxylicious.kafka.common.errors.ListenerNotFoundException;
import io.kroxylicious.kafka.common.errors.LogDirNotFoundException;
import io.kroxylicious.kafka.common.errors.MemberIdRequiredException;
import io.kroxylicious.kafka.common.errors.MismatchedEndpointTypeException;
import io.kroxylicious.kafka.common.errors.NetworkException;
import io.kroxylicious.kafka.common.errors.NewLeaderElectedException;
import io.kroxylicious.kafka.common.errors.NoReassignmentInProgressException;
import io.kroxylicious.kafka.common.errors.NotControllerException;
import io.kroxylicious.kafka.common.errors.NotCoordinatorException;
import io.kroxylicious.kafka.common.errors.NotEnoughReplicasAfterAppendException;
import io.kroxylicious.kafka.common.errors.NotEnoughReplicasException;
import io.kroxylicious.kafka.common.errors.NotLeaderOrFollowerException;
import io.kroxylicious.kafka.common.errors.OffsetMetadataTooLarge;
import io.kroxylicious.kafka.common.errors.OffsetMovedToTieredStorageException;
import io.kroxylicious.kafka.common.errors.OffsetNotAvailableException;
import io.kroxylicious.kafka.common.errors.OffsetOutOfRangeException;
import io.kroxylicious.kafka.common.errors.OperationNotAttemptedException;
import io.kroxylicious.kafka.common.errors.OutOfOrderSequenceException;
import io.kroxylicious.kafka.common.errors.PolicyViolationException;
import io.kroxylicious.kafka.common.errors.PositionOutOfRangeException;
import io.kroxylicious.kafka.common.errors.PreferredLeaderNotAvailableException;
import io.kroxylicious.kafka.common.errors.PrincipalDeserializationException;
import io.kroxylicious.kafka.common.errors.ProducerFencedException;
import io.kroxylicious.kafka.common.errors.ReassignmentInProgressException;
import io.kroxylicious.kafka.common.errors.RebalanceInProgressException;
import io.kroxylicious.kafka.common.errors.RebootstrapRequiredException;
import io.kroxylicious.kafka.common.errors.RecordBatchTooLargeException;
import io.kroxylicious.kafka.common.errors.RecordTooLargeException;
import io.kroxylicious.kafka.common.errors.ReplicaNotAvailableException;
import io.kroxylicious.kafka.common.errors.ResourceNotFoundException;
import io.kroxylicious.kafka.common.errors.RetriableException;
import io.kroxylicious.kafka.common.errors.SaslAuthenticationException;
import io.kroxylicious.kafka.common.errors.SecurityDisabledException;
import io.kroxylicious.kafka.common.errors.ShareSessionLimitReachedException;
import io.kroxylicious.kafka.common.errors.ShareSessionNotFoundException;
import io.kroxylicious.kafka.common.errors.SnapshotNotFoundException;
import io.kroxylicious.kafka.common.errors.StaleBrokerEpochException;
import io.kroxylicious.kafka.common.errors.StaleMemberEpochException;
import io.kroxylicious.kafka.common.errors.StreamsInvalidTopologyEpochException;
import io.kroxylicious.kafka.common.errors.StreamsInvalidTopologyException;
import io.kroxylicious.kafka.common.errors.StreamsTopologyFencedException;
import io.kroxylicious.kafka.common.errors.TelemetryTooLargeException;
import io.kroxylicious.kafka.common.errors.ThrottlingQuotaExceededException;
import io.kroxylicious.kafka.common.errors.TimeoutException;
import io.kroxylicious.kafka.common.errors.TopicAuthorizationException;
import io.kroxylicious.kafka.common.errors.TopicDeletionDisabledException;
import io.kroxylicious.kafka.common.errors.TopicExistsException;
import io.kroxylicious.kafka.common.errors.TransactionAbortableException;
import io.kroxylicious.kafka.common.errors.TransactionCoordinatorFencedException;
import io.kroxylicious.kafka.common.errors.TransactionalIdAuthorizationException;
import io.kroxylicious.kafka.common.errors.TransactionalIdNotFoundException;
import io.kroxylicious.kafka.common.errors.UnacceptableCredentialException;
import io.kroxylicious.kafka.common.errors.UnknownControllerIdException;
import io.kroxylicious.kafka.common.errors.UnknownLeaderEpochException;
import io.kroxylicious.kafka.common.errors.UnknownMemberIdException;
import io.kroxylicious.kafka.common.errors.UnknownProducerIdException;
import io.kroxylicious.kafka.common.errors.UnknownServerException;
import io.kroxylicious.kafka.common.errors.UnknownSubscriptionIdException;
...
import io.kroxylicious.kafka.common.errors.UnreleasedInstanceIdException;
import io.kroxylicious.kafka.common.errors.UnstableOffsetCommitException;
import io.kroxylicious.kafka.common.errors.UnsupportedAssignorException;
import io.kroxylicious.kafka.common.errors.UnsupportedByAuthenticationException;
import io.kroxylicious.kafka.common.errors.UnsupportedCompressionTypeException;
import io.kroxylicious.kafka.common.errors.UnsupportedEndpointTypeException;
import io.kroxylicious.kafka.common.errors.UnsupportedForMessageFormatException;
import io.kroxylicious.kafka.common.errors.UnsupportedSaslMechanismException;
import io.kroxylicious.kafka.common.errors.UnsupportedVersionException;
import io.kroxylicious.kafka.common.errors.VoterNotFoundException;
...
public enum Errors {
    ...
    TRANSACTION_ABORTABLE(120, "The server encountered an error with the transaction. The client can abort the transaction to continue using this transactional ID.",
            TransactionAbortableException::new),
