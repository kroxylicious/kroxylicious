...
public enum ControlRecordType {
    ...
    ControlRecordType(short type) {
        this.type = type;
        // TODO #4578: replace with ControlRecordTypeSchema once message generator produces it
        // The schema is: Version (Int16=0) + Type (Int16), total 4 bytes.
        ByteBuffer buf = ByteBuffer.allocate(CONTROL_RECORD_KEY_SIZE);
        buf.putShort((short) 0); // version = HIGHEST_SUPPORTED_VERSION = 0
        buf.putShort(type);
        buf.flip();
        buffer = buf;
    }
    ...
    public static short parseTypeId(ByteBuffer key) {
        ...
        // TODO #4578: replace inline parsing with ControlRecordTypeSchema once message generator produces it
        // Schema version 0: Version (Int16) + Type (Int16). LOWEST=HIGHEST=0.
        short version = buffer.getShort();
        if (version < 0) // LOWEST_SUPPORTED_VERSION = 0
            throw new InvalidRecordException("Invalid version found for control record: " + version +
                    ". May indicate data corruption");

        if (version > 0) { // HIGHEST_SUPPORTED_VERSION = 0
            log.debug("Received unknown control record key version {}. Parsing as version {}", version, (short) 0);
        }
        return buffer.getShort(); // type field
    }
